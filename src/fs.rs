...
pub fn run(args: &Vec<String>) {
    ...
    let sstr = sstrs.join(" ");
    let opt = opts.join(" ");
    let file_pos_cmd = format!(r#"rg --color=always -n {} "{}" | fzf -e --tac -0 -1 --color=bg+:24 --cycle --min-height=20 -d ':' --preview="echo '\033[1;32m  {{1}}\033[0m'; fspreview {{}} {}" --preview-window=right:60% | gawk -F':' '{{printf "%s +%s", $1, $2}}'"#, opt, sstr, h);
